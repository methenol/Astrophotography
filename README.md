...
| **ImageMM restoration** (option) | **ImageMM** (Sukurdeep et al. 2025, arXiv:2501.03002), as published. One non-negative, background-subtracted sky image is fitted to **every individual sub at once**. Each sub has its own PSF measured from its stars, photon-transfer variances and masks. The fit uses majorization-minimization, with Huber-robust weights (removes satellite trails) and the paper's update clipping and stopping rule. The sky background is driven to zero instead of being denoised. There is optional 2× super-resolution with the paper's variational PSF refinement. The subs are prepared by linear demosaicing and star-refined registration (RMS at the centroid-noise level), with per-colour photometric scales and background models. Optional extras are Biggs–Andrews acceleration, seeing-group coadds, Moffat PSF models and a Noise2Noise pass. The same multi-frame likelihood can also replace the AI deconvolution network's data term ("Multi-frame loss groups"). On held-out subs of M 27 it predicts every sub about 25% better than the coadd, and sky noise falls by three orders of magnitude (`experiments/README.md`). |
